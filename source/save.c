...
void SaveSymCodeInfo(MFILE fil, void *ptr)    
    ...
    if (!ptr)
        ...
        {
        #ifdef SW_3DFX 
        // SymCodeSegOffset is incorrect for 3dfx compiles due to segment
        // differences. Hacked a fix in.
        SymCodeSegOffset = -((unsigned long)((char*)TimerFunc - ((char *)(&__begtext))));
        #endif
...
int SaveGame(short save_num)
{
    ...
    #ifdef SW_3DFX
    LoadSymTable("fxdata.sym", &SymTableData, &SymCountData);
    LoadSymTable("fxcode.sym", &SymTableCode, &SymCountCode);
    ...
    #ifdef SW_3DFX
    sprintf(game_name,"game%d.sa2",save_num);
    #else
...
int LoadGame(short save_num)
{
    ...
    set_pal(Player[myconnectindex].temp_pal);
    COVERsetbrightness(gs.Brightness,(char *)palette);
    COVERsetbrightness(gs.Brightness,(char *)palette_data);
