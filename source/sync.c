...
getsyncstat(void)
	...
	// TENSW: do not fill the sync fifo if I'm the only player
	if (numplayers < 2)
		return;

    #if TEN_DEBUG    
	// check for collision with end of queue for outgoing packets
	tenDbVerify(pp->syncvalhead - syncvaltail < SYNCFIFOSIZ - 1);
	// check for collision with end of queue for incoming sync checks
	tenDbVerify(pp->syncvalhead - syncvaltottail < SYNCFIFOSIZ - 1);
    #endif
	...
	count += val;
    #if TEN_DEBUG    
	tenDbLprintf(gTenLog, 8, "sync %ld %lx %ld", (long) pp->syncvalhead, val, count);
    #endif
...
SyncStatMessage(void)
    ...
    for (i = 0; i < NumSyncBytes; i++)
        ...
        if (syncstat[i] != 0)
            {
            #if TEN_DEBUG
			//tenDbLprintf(gTenLog, 5, "out of sync");
            #endif
            if (NumSyncBytes > 1) 
                {
...
GetSyncInfoFromPacket(char *packbuf, long packbufleng, long *j, long otherconnectindex)
    ...
    while ((*j) < packbufleng)  
        ...
        #if TEN_DEBUG    
	 	// check for collision with end of queue for outgoing packets
		tenDbVerify(ppo->syncvalhead - syncvaltail < SYNCFIFOSIZ);
		// check for collision with end of queue for incoming sync checks
		tenDbVerify(ppo->syncvalhead - syncvaltottail < SYNCFIFOSIZ);
        #endif
       }
        ...
        for (i = connectpoint2[connecthead]; i >= 0; i = connectpoint2[i])
            ...
            for (sb = 0; sb < NumSyncBytes; sb++)
                ...
                if (Player[i].syncval[syncvaltottail & (SYNCFIFOSIZ - 1)][sb] != Player[connecthead].syncval[syncvaltottail & (SYNCFIFOSIZ - 1)][sb])
                    {
                    #if TEN_DEBUG
					tenDbLprintf(gTenLog, 5, "sync calc failed %lx != %lx, index %ld  sb %d  i %d",
						(long) (Player[i].syncval[syncvaltottail & (SYNCFIFOSIZ - 1)][sb]), 
						(long) (Player[connecthead].syncval[syncvaltottail & (SYNCFIFOSIZ - 1)][sb]),
						(long) syncvaltottail, (int) sb, (int) i);
                    #endif        
                    syncstat[sb] = 1;
                    }
                }    
