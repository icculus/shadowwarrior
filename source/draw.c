...
#ifdef __3DFX_ACC__
extern int renderOnlyBackBuffer;
#endif
...
#ifdef __3DFX_ACC__
extern int use512x384;
extern int glideXRes;
extern int glideYRes;
#endif
...
long COVERsetgamemode(long mode, long xdim, long ydim)
    {
    extern long ScreenHeight, ScreenWidth, ScreenMode;
    ...
    if (xdim == 512)
      {
        ...
        xdim = 512;
        ydim = 384;
        // Can't set 640x480, but fool build into thinking that.
		return((long)setgamemode(mode,640,480));
      }
    else
      {
	// Should only open up 640x480 context
        return((long)setgamemode(mode,xdim,ydim));
      }
    #else
    return((long)setgamemode(mode,xdim,ydim));
    #endif
...
void ResChange(void)
	...
	// CTW ADDITION
	// This prevents the res change from going through. If someone wants
	// to come in and fix up this code, knock yourself out.
	CON_Message("F5: Quick Resultion Change Feature Disabled.");
	return;
	// CTW ADDITION END

#ifdef __3DFX_ACC__
//    sprintf(ds,"Function not Supported in 3Dfx version");
	...
	sprintf ( ds, "%d x %d mode selected.", glideXRes, glideYRes );
...
VOID ScreenCaptureKeys(VOID)
    ...
    if (KEY_PRESSED(KEYSC_F12))
        ...
        PauseAction();
#ifdef __3DFX_ACC__
        // 3Dfx screen captures are tga files
        screencapture("swcpxxxx.tga", KEY_PRESSED(KEYSC_LSHIFT) | KEY_PRESSED(KEYSC_LSHIFT));
#else
...
#ifdef __3DFX_ACC__
        // Turn off bilinear for help screens */
