...
/*
option[4] - network option
0 = single play
1-4 = comm 1-4
5-11 = Network 2-8 player
*/
...
long chainyres[11] = {200, 240, 256, 270, 300, 350,
    360, 400, 480, 512, 540};
long vesares[7][2] = {320, 200, 640, 400, 640, 480,
    800, 600, 1024, 768,
    1280, 1024, 1600, 1200};
*/
...
void Set_GameMode(void)
    {
    long COVERsetgamemode(long,long,long);
    extern long ScreenMode, ScreenWidth, ScreenHeight;
    int result;
    char ch;
    ...
    switch ( ScreenWidth ) 
        ...
        case 512:
            use512x384 = TRUE;
            setGlideWindow ( 512, 384 );
        break;
        case 640:
            setGlideWindow ( 640, 480 );
        break;
        case 800:
            setGlideWindow ( 800, 600 );
        break;
        }
        
    COVERsetgamemode ( 1, glideXRes, glideYRes );
    return;
    #endif
    
    //DSPRINTF(ds,"ScreenMode %d, ScreenWidth %d, ScreenHeight %d",ScreenMode, ScreenWidth, ScreenHeight);
    //MONO_PRINT(ds);
    result = COVERsetgamemode(ScreenMode, ScreenWidth, ScreenHeight);
...
InitNewGame(VOID)
    ...
    long ver_bak;
        ...
        //tenDbLprintf(gTenLog, 3, "InitNewGame wfe");
...
void FindLevelInfo(char *map_name, short *level)
    ...
    for (j = 1; j <= MAX_LEVELS; j++)
        ...
        if (LevelInfo[j].LevelName)
            {
            if (stricmp(map_name, LevelInfo[j].LevelName) == 0)
                {
                *level = j;
                return;
                }
...
InitLevel(VOID)
    ...
    if (stricmp(CacheLastLevel, LevelName) != 0)
...
LogoLevel(VOID)
    ...
    while (TRUE)
        ...
        if (totalclock > 5*120 || KeyPressed() /* CTW REMOVED || gTenActivated */ )
            {
            break;
            }
        }
...
IntroAnimLevel(VOID)
    {
      ...
      MONO_PRINT(ds);
#ifdef __3DFX_ACC__
      /* This converts the textures for all of the animations */
      playanm3dfxcache(ANIM_INTRO);
      playanm3dfxcache(ANIM_SERP);
      playanm3dfxcache(ANIM_SUMO);
      playanm3dfxcache(ANIM_ZILLA);
      
      /* This is for testing all the animations */ 
      // playanm(ANIM_INTRO);
      // playanm(ANIM_SERP);
      // playanm(ANIM_SUMO);
      // playanm(ANIM_ZILLA);
...
GameIntro(VOID)
    ...
    if (!AutoNet /* CTW REMOVED && !gTenActivated */)
        ...
        LogoLevel();
#ifdef __3DFX_ACC__
        /* This is to get these things to be converted before 
           converting the big textures */
        TitleLevel();
        DrawMenuLevelScreen();
        clearview(0);
        nextpage();
        // This will convert the textures that are > 256x256 */
        preCacheBigTextures();
#endif
        //CreditsLevel();
        //SceneLevel();
...
void MoveLoop(void)    
    ...
    long pnum;
        ...
        // TENSW: moved getpackets to start of loop to put prediction closer to move
        // handling
    ...
    while (Player[myconnectindex].movefifoend - movefifoplc > bufferjitter)
        {
         ...
         for(pnum=connecthead;pnum>=0;pnum=connectpoint2[pnum])
            ...
            if (movefifoplc == Player[pnum].movefifoend)
								...
								// CTW REMOVED
                                //if (gTenActivated)
                                //        tenJitterStarve(pnum);          // used to tweak the jitter value
								// CTW REMOVED END
                break;
                                }
            }    
...
VOID InitRunLevel(VOID)
    ...
    if (LoadGameOutsideMoveLoop)
        ...
        //tenDbLprintf(gTenLog, 3, "preSendVersion wfe");
        waitforeverybody();
    #endif
...
#ifndef NOMEMCHECK 
void _MCCALLBACK erf_custom(char *message)
{
    erf_log_only(message);
    //printf("%s\n",message);
    MONO_PRINT(message);
}
#endif

void move_cursor(long row, long column);

#pragma aux move_cursor =\
        "mov dh, al",\
        "mov dl, bl",\
        "xor eax,eax",\
        "mov ah, 2",\
...
VOID AlphaMessage(VOID)
    ...
    _outtext(""
    #ifdef SW_3DFX
    "                    SHADOW WARRIOR(tm) Version 1.2 (3Dfx v1.1)                \n"
    #else
...
VOID AlphaMessage(VOID)
    ...
    _settextcolor(15);
    _outtext(""
"                       SHADOW WARRIOR(tm) BETA Version 1.2                     \n");
...
"                James Barnes/FPGAMING 1(510)494-2033 on 7/8/97.\n\n"
"                   DO NOT DISTRIBUTE this Beta version!    \n\n"
"                      <Press any key to continue> \n");
    _outtext("\n\n\n");
...
VOID AlphaMessage(VOID)
    ...
    _outtext(""
"              This is a version of Shadow Warrior(tm) supplied to \n"
"                    Johnathan Mendoza/SYBEX on 8/10/97.\n\n"
"                   DO NOT DISTRIBUTE this Beta version!    \n\n"
"                      <Press any key to continue> \n");
    _outtext("\n\n\n");
...
VOID AlphaMessage(VOID)
    ...
    _outtext("\n\n\n");
    _outtext(""
//    "      This version compiled on 7/18/97 for the Marcus Fusilero at \n"
//    "                  Spacetec IMG.  DO NOT DISTRIBUTE!\n\n"
//    "                      <Press any key to continue> \n");
    "      This version compiled on 8/10/97 for the Geoff Keighly at \n"
    "       (310) 230-3509 16746 Calle Alicante, Pacific Palisades, CA\n"
    "                            DO NOT DISTRIBUTE!\n\n"
    "                      <Press any key to continue> \n");
    _outtext("\n\n\n");
...
VOID AlphaMessage(VOID)
    ...
    _outtext(""
"              This is a version of Shadow Warrior(tm) supplied to \n"
"                      Rick Raymo/GT Interactive 8/10/97.\n\n"
"                   DO NOT DISTRIBUTE this Beta version!    \n\n"
"                      <Press any key to continue> \n");
    _outtext("\n\n\n");
...
VOID AlphaMessage(VOID)
    ...
    _settextcolor(15);
    _outtext(""
    "                     SHADOW WARRIOR(tm) BETA #3  Version 1.2                  \n"
    "                    Copyright (c) 1997 3D Realms Entertainment                \n");
    ...
    _outtext("\n\n\n");
    _outtext(""
    "      This version compiled on 08/10/97 for the internal beta test team \n"
    "        at 3D Realms.  Distribution beyond the group is prohibited.\n\n"
    "                      <Press any key to continue> \n");
    _outtext("\n\n\n");
...
VOID AlphaMessage(VOID)
    ...
    _outtext("\n\n\n");
//    _outtext(""
//    "       This is a BETA version of Shadow Warrior(tm).  DO NOT DISTRIBUTE!    \n\n"
//    "                     This is a 3DRealms in house copy.\n\n");    
...
main(short int argc, char *argv[])
    ...
    for (cnt = 1; cnt < argc; cnt++)
        ...
        CON_StoreArg(argv[cnt]);

        if (memcmp(argv[cnt], "-autonet",8) == 0)
            {   
            AutoNet = TRUE;
            cnt++;
        ...
        if (memcmp(argv[cnt], "-turnscale",10) == 0)
            {
            if (cnt <= argc-2)
                {
                cnt++;
        ...
        if (memcmp(argv[cnt], "-movescale",10) == 0)
            {
            if (cnt <= argc-2)
                {
                cnt++;
        ...
        if (memcmp(argv[cnt], "-extcompat",10) == 0)
            {
            move_scale *= 5; 
            turn_scale *= 5;
            }
        else
        if (memcmp(argv[cnt], "-cacheprint",11) == 0)
            {
            CachePrintMode = TRUE;
            }
        else    
        if (memcmp(argv[cnt], "-setupfile",9) == 0)
            {
            // Passed by setup.exe
            // skip setupfile name
            cnt++;
        ...
        if (memcmp(argv[cnt], "-net",4) == 0)
            {
            // Passed my commit.exe
            // skip net address number
            cnt++;
            }
        else    
        if (memcmp(argv[cnt], "control",7) == 0)
            {
            // Passed by external.exe?
            // skip net address number
            cnt++;
            }
        else    
        if (memcmp(argv[cnt], "-control",8) == 0)
            {
            // Passed by external.exe?
            // skip net address number
            cnt++;
        ...
        if (memcmp(argv[cnt], "-short",6) == 0)
            {
            ShortGameMode = TRUE;
            }
        else
        if (memcmp(argv[cnt], "-nodemo",7) == 0)
            {
            NoDemoStartup = TRUE;
            }
        else
        if (memcmp(argv[cnt], "-allsync",4) == 0)
            {
            NumSyncBytes = 8;
            }
        else
        if (memcmp(argv[cnt], "-name",5) == 0)
            {
            if (cnt <= argc-2)
                {
        ...
        if (memcmp(argv[cnt], "-f8",3) == 0)
            {
            MovesPerPacket = 8;
            }
        else
        if (memcmp(argv[cnt], "-f4",3) == 0)
            {
            MovesPerPacket = 4;
            }
        else
        if (memcmp(argv[cnt], "-f2",3) == 0)
            {
            MovesPerPacket = 2;
            }
        else
                if (memcmp(argv[cnt], "-monst", 6) == 0)
            {
            DebugActor = TRUE;
            }
        else
        if (memcmp(argv[cnt], "-nopredict",9) == 0)
            {
            extern BOOL PredictionOn;
            PredictionOn = FALSE;
            }
                else
                if (memcmp(argv[cnt], "-ten", 4) == 0)          // TENSW
                        {
						// CTW REMOVED
						//gTenActivated = true;
						// CTW REMOVED END
        ...
        if (memcmp(argv[cnt], "-col", 4) == 0)
            ...
            sscanf(argv[cnt],"%d",&temp);
                ...
                if (memcmp(argv[cnt], "-level", 6) == 0)
            {
            if (strlen(argv[cnt]) > 6)
                {
                strcpy(UserMapName,LevelInfo[atoi(&argv[cnt][6])].LevelName);
                }
            }
        else
        if (memcmp(argv[cnt], "-s", 2) == 0)
            {
            if (strlen(argv[cnt]) > 2)
                Skill = atoi(&argv[cnt][2])-1;
            
            Skill = max(Skill,0);
            Skill = min(Skill,3);
            }
        else
                if (memcmp(argv[cnt], "-commbat", 8) == 0)
            {
            if (strlen(argv[cnt]) > 8)
                {
                FakeMultiNumPlayers = atoi(&argv[cnt][8]);
                gNet.MultiGameType = MULTI_GAME_COMMBAT;
                }
            }
        else
        ...
        #endif
        */
                if (memcmp(argv[cnt], "-nometers", 9) == 0)
            {
            NoMeters = TRUE;
            }
        else 
            ...
            if (strlen(argv[cnt]) > 5)
                ...
                if (FALSE && memcmp(argv[cnt], "-ddr", 4) == 0)
            {
            //NumSyncBytes = 8;
            DemoRecording = TRUE;
            DemoPlaying = FALSE;
            ...
            if (strlen(argv[cnt]) > 3)
                {
                strcpy(DemoFileName, &argv[cnt][3]);
                if (strchr(DemoFileName, '.') == 0)
                    strcat(DemoFileName, ".dmo");
                }    
            }
        else
                if (FALSE && memcmp(argv[cnt], "-dr", 3) == 0)
            {
            //NumSyncBytes = 8;
            DemoRecording = TRUE;
            DemoPlaying = FALSE;
            ...
            if (strlen(argv[cnt]) > 3)
                ...
                if (FALSE && memcmp(argv[cnt], "-dp", 3) == 0)
            {
            DemoPlaying = TRUE;
            DemoRecording = FALSE;
            PreCaching = TRUE;
        ...
        if (memcmp(argv[cnt], "-masterslave",7) == 0)
            {
            NetModeOverride = TRUE;
            NetBroadcastMode = FALSE;
            }
        else
        if (memcmp(argv[cnt], "-broadcast",6) == 0)
            {
            NetModeOverride = TRUE;
            NetBroadcastMode = TRUE;
            }
        ...
        if (memcmp(argv[cnt], "-cheat",6) == 0)
            {
            ArgCheat = TRUE;
            }
        else    
        if (memcmp(argv[cnt], "-demosynctest",13) == 0)
            {
            NumSyncBytes = 8;
            DemoSyncTest = TRUE;
        ...
        if (memcmp(argv[cnt], "-demosyncrecord",13) == 0)
            {
            NumSyncBytes = 8;
            DemoSyncTest = FALSE;
            DemoSyncRecord = TRUE;
        ...
        if (memcmp(argv[cnt], "-cam",4) == 0)
            ...
            CameraTestMode = TRUE;
                ...
                if (FALSE && memcmp(argv[cnt], "-de", 4) == 0)
            {
        ...
        if (memcmp(argv[cnt], "-randprint",6) == 0)
            {
            RandomPrint = TRUE;
            }
        else
            ...
            if (strlen(argv[cnt]) > 6)
                ...
                if (memcmp(argv[cnt], "-debugsecret", 11) == 0)
            {
            extern BOOL DebugSecret;
            DebugSecret = TRUE;
            }
        else    
                if (memcmp(argv[cnt], "-debugactor", 11) == 0)
            {
            DebugActor = TRUE;
            }
        else
                if (memcmp(argv[cnt], "-mono", 5) == 0)
            {
            DispMono = TRUE;
            }
        else
                if (memcmp(argv[cnt], "-debugso", 8) == 0)
            {
            DebugSO = TRUE;
            }
        else
        if (memcmp(argv[cnt], "-nosyncprint",11) == 0)
            {
            extern BOOL SyncPrintMode;
            SyncPrintMode = FALSE;
            }
        else
                if (memcmp(argv[cnt], "-debuganim", 10) == 0)
            {
            DebugAnim = TRUE;
            }
        else
                if (memcmp(argv[cnt], "-debugsector", 12) == 0)
            {
            DebugSector = TRUE;
            }
        else
                if (memcmp(argv[cnt], "-debugpanel", 11) == 0)
            {
            DebugPanel = TRUE;
            }
        else
                if (FALSE && memcmp(argv[cnt], "-dt", 3) == 0)
            {
            if (strlen(argv[cnt]) > 3)
                {
                strcpy(DemoTmpName, &argv[cnt][3]);
                if (strchr(DemoFileName, '.') == 0)
                    strcat(DemoFileName, ".dmo");
                }    
            }
    ...
    TotalMemory = Z_AvailHeap();
    if (TotalMemory < MEG(9) - 4500000)
        {
        printf("\n\nError: Not enough memory.\n");
        printf("Shadow Warrior requires 9 MB free memory to execute!\n\n");
...
FunctionKeys(PLAYERp pp)
    ...
    // This is used for toggling bilinear
    if (KEY_PRESSED(KEYSC_ALT) || KEY_PRESSED(KEYSC_RALT))
    {
        if (KEY_PRESSED(KEYSC_P))
        {
           setGlideStateBilinear(0);
        }
        else 
        {
           if (KEY_PRESSED(KEYSC_B))
           {
              setGlideStateBilinear(1);
           }
        }
        SetRedrawScreen(pp);
    }
    ...
    if (KEY_PRESSED(KEYSC_F11) > 0)
        ...
        if(!pp->NightVision && pp->FadeAmt <= 0)
            {
            COVERsetbrightness(gs.Brightness,(char *)&palette[0]);
...
#define SET_LOC_KEY(loc, sync_num, key_test) SET(loc, ((!!(key_test)) << (sync_num)))
    ...
    extern BOOL MenuButtonAutoAim;
        ...
        // TENSW
    if ((Prediction && CommEnabled) /* CTW REMOVED || gTenActivated */) 
        {
        newpp = ppp;
        }
    ...
    switch (ControllerType)
        ...
        case controltype_keyboardandexternal:
            ...
            if (info.dpitch)    
                {
                // make sure its turned on for the test
                SET(pp->Flags, PF_MOUSE_AIMING_ON);
                gs.MouseAimingOn = TRUE;
