...
// TENSW
// hook send so that TEN can handle packets in TEN mode
...
// TENSW
// natively handle broadcasts under TEN
...
// switch to TEN API if TEN is in use
...
SendVersion(long version)
	...
	//tenDbLprintf(gTenLog, 3, "send version %lx", (long) version);
...
CheckVersion(long GameVersion)
    ...
    TRAVERSE_CONNECT(pnum)
        ...
        if (pnum != myconnectindex)
            {
			//tenDbLprintf(gTenLog, 3, "check version %lx ?= pid %d ver %lx", (long) GameVersion, (int) pnum, (long) Player[pnum].PlayerVersion);
...
getpackets(VOID)
    ...
    while ((packbufleng = netgetpacket(&otherconnectindex, packbuf)) > 0)
		...
		case SERVER_GENERATED_BROADCAST:
            ...
            if ((Player[otherconnectindex].movefifoend & (TIMERUPDATESIZ - 1)) == 0 /* CTW REMOVED && !gTenActivated */)
                {
...
/*
			...
			if (pp->movefifoend - movefifoplc >= MOVEFIFOSIZ - 1)
			{
				// FIFO queue overflow

				tenDbLprintf(gTenLog, 5, "FIFO overflow: %d", (int) (pp->movefifoend - movefifoplc));
				tenDbLprintf(gTenLog, 5, "movefifoplc: %ld", movefifoplc);

				for (i = connecthead; i >= 0; i = connectpoint2[i])
				{
					tenDbLprintf(gTenLog, 5, "pid %d mfe %ld", (int) i, (long) pp->movefifoend);
				}
				tenDbVerify(pp->movefifoend - movefifoplc < MOVEFIFOSIZ - 1);
			}
...
#if TEN_DEBUG
			{
				int otherFlag, myFlag;
				
				otherFlag = Player[otherconnectindex].playerreadyflag;
				myFlag = Player[myconnectindex].playerreadyflag;

				// check it was what I'm expecting you to be on
				tenDbVerify(otherFlag == packbuf[1]);
				// check we're locked on the same w4e (I might not have called it yet)
				tenDbVerify(otherFlag == myFlag || otherFlag == myFlag + 1);
			}
#endif
*/
// CTW REMOVED END

			//tenDbLprintf(gTenLog, 3, "got player %d ready, prf %d", (int) otherconnectindex,(int) Player[otherconnectindex].playerreadyflag);
