...
#ifdef __3DFX_ACC__
// This is just to cache the animation to disk for the
// 3dfx version.
void 
playanm3dfxcache(short anim_num)
{
  char *animbuf, *palptr;
  ...
  {
    struct stat sbuf;
    sprintf(tempbuf,"anim%d",ANIM_TILE(ANIMnum));
    if (stat(tempbuf,&sbuf) != -1) return;
  }
#endif
...
#define CLEAR_LINE 32
#ifdef WIEDER_HACK_REMOVE
  glideWriteString(25,line+=8,"Converting movie animation to 3DFX format");
  glideWriteString(25,line+=8,"This could take a while...");
#endif
  sprintf(ds,"If you want to recreate this process, delete the anim%d subdirectory.",ANIM_TILE(ANIMnum));
#ifdef WIEDER_HACK_REMOVE
  glideWriteString(25,line+=8,ds);
  glideWriteString(25,line+=8,"under the game directory...");
#endif
  
  for (i = 1; i < numframes; i++)
    {
      animationFrameNumber = i;
      waloff[ANIM_TILE(ANIMnum)] = FP_OFF(ANIM_DrawFrame(i));
      if (i%CLEAR_LINE == 0) {
        clearview(0);
        nextpage();
#ifdef WIEDER_HACK_REMOVE
        glideWriteString(25,18,"Converting movie animation to 3DFX format");
        glideWriteString(25,26,"This could take a while...");
#endif
        sprintf(ds,"If you want to recreate this process, delete the anim%d subdirectory.",ANIM_TILE(ANIMnum));
#ifdef WIEDER_HACK_REMOVE
        glideWriteString(25,34,ds);
        glideWriteString(25,42,"under the game directory...");
#endif
      }
      // Print a status to the screen
      sprintf(ds,"Converting animation texture %d frame %d",ANIM_TILE(ANIMnum),i);
#ifdef WIEDER_HACK_REMOVE
      glideWriteString(25,((i%CLEAR_LINE)*8)+line,ds);
#endif
      rotatesprite(0 << 16, 0 << 16, 65536L, 512, ANIM_TILE(ANIMnum), 0, 0, 2 + 4 + 8 + 16 + 64, 0, 0, xdim - 1, ydim - 1);
    }
...
playanm(short anim_num)
    ...
    for (i = 1; i < numframes; i++)
        {
#ifdef __3DFX_ACC__
          // the 3dfx version needs to know the frame of animation we're on
          // so it can load it from the disk.
          animationFrameNumber = i;
#endif
        while (totalclock < ototalclock)
            {
