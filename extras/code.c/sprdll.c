...
VOID __export  ChangeState(short SpriteNum, STATEp statep)
    {
    USERp u = User[SpriteNum];
...
DoGet(short SpriteNum)
    ...
    TRAVERSE_CONNECT(pnum)
        ...
        cstat_bak = sp->cstat;
        SET(sp->cstat, CSTAT_SPRITE_BLOCK|CSTAT_SPRITE_BLOCK_HITSCAN);
        can_see = FAFcansee(pp->posx, pp->posy, pp->posz, pp->cursectnum,
                    sp->x, sp->y, sp->z, sp->sectnum);
        sp->cstat = cstat_bak;        
...
//            pp->WpnAmmo[WPN_SHOTGUN] += 10;
        ...
        case ICON_HEART:
            ...
            PutStringInfo(Player+pnum, ds);
            PlayerUpdateAmmo(pp, WPN_HEART, 3);
            SetFadeAmt(pp,ITEMFLASHAMT,ITEMFLASHCLR);  // Flash blue on item pickup    
            if (pp == Player+myconnectindex)
                PlaySound(DIGI_ITEM, &sp->x, &sp->y, &sp->z, v3df_dontpan);
            //ChoosePlayerGetSound(pp);
